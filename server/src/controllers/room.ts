...
import { TRoomsInfo } from "../utils/types";
...
class RoomController extends BaseController {
  ...
  public getRooms = async (userName: string, done: (list: TRoomsInfo[]) => void) => {
    ...
    try {
      const rooms = await this.service.getRooms();
      if (rooms.length) this.activateUser(userName, rooms);
      done(rooms);
    } catch (err) {
      this.sendError(err);
    }
  ...
  public activateUser = (userName: string, rooms: TRoomsInfo[]) => {
    rooms.forEach((room) => {
      if (room.attendee.includes(userName)) this.socket.join(room.name);
    });
  };
}
